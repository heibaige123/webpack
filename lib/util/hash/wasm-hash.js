...
/**
 * 65536 - 64 = 65472 (可用内存空间)
 * 65472 / 4 = 16368 (考虑 UTF-8 编码后的最大字符数)
 * 16368 & ~3 (向下取整到最近的 4 的倍数)
 *
 * 65536 (64KB) - 一个 WASM 内存页的标准大小， 这是 WebAssembly 规范定义的最小内存分配单位
 * 64 字节 - 为哈希实现预留的最大 chunk 大小， 从总页面大小中减去这个值作为安全边界
 * /4 - UTF-8 编码中一个字符最多占用 4 字节， 确保在最坏情况下也能正确处理所有 Unicode 字符
 * & ~3 - 执行按 4 对齐的位操作， 确保结果总是 4 的倍数，避免 Base64 编码时的填充问题
 */
...
class WasmHash {
	...
	/**
	 * 将 JavaScript 字符串编码并传输到 WebAssembly 内存中
